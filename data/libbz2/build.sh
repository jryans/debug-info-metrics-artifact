...
make -f Makefile-libbz2_so \
  CC="${DBGCOV_CC}" \
  CFLAGS="${DEFAULT_OPTS} -save-temps ${DBGCOV_OPTS} ${CC_SYSROOT_OPTS}"
...
make -f Makefile-libbz2_so \
  CC=wllvm \
  CFLAGS="${DEFAULT_OPTS} ${CC_COMMON_OPTS} ${CC_O0_OPTS}"
...
for i in ${!levels[*]}; do
  ...
  make -f Makefile-libbz2_so \
    ...
    CFLAGS="${DEFAULT_OPTS} ${CC_COMMON_OPTS} ${!cc_level_opts}"
